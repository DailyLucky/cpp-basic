...
int SpeackerCompetition(int times, vector<int> &v_pre, map<int, Speaker> &m, vector<int> &v_next) {
	...
	for (int i=0; i<v_pre.size(); ++i) {	
		...
		score_temp.pop_back();
		m[v_pre[i]].m_score = accumulate(score_temp.begin(), score_temp.end(), 0) / score_temp.size();
		score_temp.clear();
		rank_map.insert(pair<int,int>(m[v_pre[i]].m_score, v_pre[i]));
...
void PrintSpeaker(const int times, const vector<int> &v, map<int, Speaker> &m) {
	...
	for (int i=0; i<v.size(); i++) {
		...
		cout << m[v[i]].m_name << "  " << m[v[i]].m_score << endl;
	}
}
// 演讲比赛框架
void SpeechGame() {
